...
import MangaLength from './MangaLength.mjs';
...
export default class Manga extends EventTarget {
    ...
    getUpdatedChaptersLength() {
        let markedChap = Engine.ChaptermarkManager.getChaptermark(this);
        if(!markedChap){return Promise.resolve({isMarked:false,len:this.length,markPosition:null});}
        return this.connector.initialize()
            ...
            .then(async (chapters) => {
                ...
                var chapPosition = chapters.indexOf(chapters.find((chapter) => chapter.id === markedChap.chapterID));
                try {
                    try{
                        var mangaLastLen = await Engine.Storage.loadConfig("mangaLastLen");
                    }catch(e){mangaLastLen = [];}
                    console.log(await mangaLastLen);
                    var lastLen = (await mangaLastLen).find((lastLen) => lastLen.id === this.id);
                    console.log(lastLen);
                    var newChaptLen = this.length - await lastLen.len;
                } catch (e) {
                    let seedLen = new MangaLength(this);
                    mangaLastLen.push(seedLen);
                    await Engine.Storage.saveConfig("mangaLastLen", mangaLastLen);
                    newChaptLen = 0;
                    console.log("created");
                }
                // console.log(chapters[0],markedChap);
                if (newChaptLen >= 0) {
                    console.log(this.title, ":");
                    console.log("         ", chapPosition > 0 ? chapPosition : "Chapter not blackbookmarked");
                    this.newChaptLen = newChaptLen;
                    return Promise.resolve({isMarked:true,len:newChaptLen,markPosition:chapPosition});
                }
            })
            .catch(error => {
                // TODO: remove log ... ?
                console.warn('_getUpdatedChapters', error);
                return Promise.resolve(this.chapterCache.length || 0);
            });
    }
    async updateLength(){
        var mangaLastLen = await Engine.Storage.loadConfig("mangaLastLen");
        if(this.newChaptLen > 0){
            let saveLen = (await mangaLastLen).map((elm)=>{
                if(elm.id == this.id) {
                    elm.len = this.length;
                }
                return elm;
            ...
            await Engine.Storage.saveConfig("mangaLastLen", saveLen);
        }
    }
    /**
     *
